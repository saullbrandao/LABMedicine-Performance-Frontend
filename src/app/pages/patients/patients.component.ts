...
export class PatientsComponent {
  ...
  constructor(
    ...
    [
      patientService.patientSaved,
      patientService.patientDeleted,
      patientService.httpError,
    ].forEach((subject) => {
      subject
        .pipe(takeUntil(this.componentDestroyed))
        .subscribe(this.toastResponse);
    });
    patientService.editingPatientLodaded
      .pipe(takeUntil(this.componentDestroyed))
      .subscribe(this.loadPatient);
    this.genderOptions = Object.values(Gender).filter((value) =>
      isNaN(Number(value))
    );
    this.maritalStatusOptions = Object.values(MaritalStatus).filter((value) =>
      isNaN(Number(value))
    );
  }
  ...
  getFormData(patient: Patient | undefined = undefined) {
    return {
      ...
      rg: [patient?.rg || '', [Validators.required, Validators.maxLength(20)]],
      maritalStatus: [
      ...
      specificCareList: [patient?.specificCareList || ''],
      healthInsurance: [patient?.healthInsurance || ''],
      insuranceNumber: [patient?.insuranceNumber || ''],
      insuranceValidity: [patient?.insuranceValidity || null],
      address: this.formBuilder.group({
        cep: [patient?.address.cep || '', [Validators.required]],
        city: [patient?.address.city || '', [Validators.required]],
        state: [patient?.address.state || '', [Validators.required]],
        street: [patient?.address.street || '', [Validators.required]],
        number: [patient?.address.number || '', [Validators.required]],
      ...
      status: [patient ? patient.status : true],
    };
  }
  ...
  isInvalid(input: string) {
    return (
      (this.form.get(input)?.invalid && this.form.get(input)?.touched) ||
      (this.formGroupAddress.get(input)?.invalid &&
        this.formGroupAddress.get(input)?.touched)
    );
  }
  ...
  toastResponse = (response: Response) => {
    ...
    if (response.status === 202 && response.message.includes('excluído'))
