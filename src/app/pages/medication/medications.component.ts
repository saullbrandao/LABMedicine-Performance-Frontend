...
export class MedicationsComponent {
  ...
  constructor(
    ...
    [
      medicationService.medicationSaved,
      medicationService.medicationDeleted,
      medicationService.httpError,
    ].forEach((subject) => {
      subject
        .pipe(takeUntil(this.componentDestroyed))
        .subscribe(this.toastResponse);
    });
    medicationService.editingMedicationLoaded
      .pipe(takeUntil(this.componentDestroyed))
      .subscribe(this.loadMedication);
    this.typeOptions = Object.values(MedicationType).filter((value) =>
      isNaN(Number(value))
    );
    this.unitOptions = Object.values(MedicationUnit)
      .filter((value) => isNaN(Number(value)))
      .map((x) => x as MedicationUnit);
  }
  ...
  getUnit(value: MedicationUnit) {
    return value.toString() === 'porcento' ? '%' : value;
  }
