...
export class AppointmentService {
  ...
  save(appointment: Appointment) {
    if (appointment.id) {
      this.http
        .put(`${this.baseUrl}/${appointment.id}`, appointment)
        .subscribe((data) => {
      ...
      this.http.post(`${this.baseUrl}`, appointment).subscribe((data) => {
        this.appointmentSaved.next({
          status: 201,
  ...
  delete(appointmentId: number) {
    this.http.delete(`${this.baseUrl}/${appointmentId}`).subscribe((data) => {
